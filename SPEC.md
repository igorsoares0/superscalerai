...
- When the preset protects text, also runs Florence-2's "OCR with
  Region" task (same model, concurrent prediction): its boxes replace
  the Analyzer's gradient heuristic in `context.text_regions` (added
  2026-07-21 — the OCR reads the actual glyph quads; the heuristic
  stays as the fallback when the OCR call fails). Florence hallucinates
  tiny non-Latin digit labels on textless images, so regions are
  filtered: >=2 real glyphs (letters/ASCII digits) and >=0.01% of image
  area (`analysis.ocr_text_regions`).
...
Face routing (by the Planner, from the Analyzer's boxes):

- At low main-pass creativity: large face (close-up) → normal generative pass handles it well; small face → zoom-and-enhance repair.
- At high main-pass creativity (>= `Planner.CLOSEUP_SAFE_CREATIVITY`, default 0.25 — a user pushing the creativity slider, or an aggressive preset for a "wow" background) → EVERY face is repaired, because a hot pass degrades even close-ups. This decouples the two dials — aggressive background, identity-safe faces (item 2, 2026-07-24). The threshold is conservative (portrait's calibrated 0.20 keeps the size-based rule unchanged); the exact cutover still wants GPU validation.

The face repair creativity is independent of the main pass (`Preset.face_creativity` / `ExecutionPlan.face_creativity`, default 0.10, calibrated 2026-07-21) and stored in the ExecutionPlan for reproducibility.
...
1. Preset calibration — build a golden set (10–20 images per category), fix the seed and sweep parameters systematically (creativity, resemblance, dynamic/HDR, steps, tile size). Calibration is most of the quality gap to Magnific-class products. STARTED 2026-07-16: harness in `validation/calibrate.py` (SFace identity + downscaled PSNR + Laplacian detail, resumable sweep); portrait calibrated on a 3-image people set — identity falls off a cliff above creativity 0.20 (SFace 0.8→0.5 at 0.28) while detail barely grows, so portrait moved to creativity 0.20 / resemblance 1.2. Zoom-and-enhance re-validated 2026-07-21 (`validation/calibrate_faces.py`, face crops in the small-face regime, no prompt as in production): identity decays monotonically with creativity (SFace 0.92 @ 0.10 vs 0.65 @ 0.25, visible skin-tone drift at 0.25) while 0.10 still adds real detail over the input — face pass moved from 0.25/0.9 to creativity 0.10 / resemblance 1.2 (also faster: ~3.6s GPU vs ~8s). HDR/steps swept 2026-07-21 at the calibrated portrait point: identity rises monotonically as hdr drops (SFace +0.07..0.09 at hdr 3 vs 6, PSNR best at 3, detail equal or better) and steps=24 beats 18 at every hdr for ~the same GPU time — portrait preset now carries hdr=3 (Preset.hdr field; other presets stay at Clarity's default 6 pending their golden sets) and the main generative pass runs 24 steps. Progressive 4x (2x -> re-caption -> 2x) A/B'd against Clarity's internal single-call 4x on 2026-07-21 (`validation/ab_progressive.py`, 768px inputs -> 3072px outputs): statistical tie on identity/PSNR/detail (differences within noise, directions mixed across images), visually equivalent, and GPU time is the SAME (~47-57s both arms — Clarity's internal 4x already pays the progressive cost). Verdict: keep the single call; the progressive loop stays a self-host concern. Still open: golden sets for product/architecture/ai-generated.
2. ~~Better captioner~~ DONE 2026-07-16 — Florence-2 replaced BLIP-2 ("Detailed Caption" level; "More Detailed" overflows SD 1.5's encoder). Still open: region-level captions (face, background) to guide each area with the right prompt. ~~OCR replacing the text heuristic~~ DONE 2026-07-21 — Florence-2 "OCR with Region" boxes replace the heuristic when the preset protects text (see Captioner); heuristic kept as call-failure fallback.
3. Identity quality gate — STARTED 2026-07-24: the zoom-and-enhance face repair pass measures SFace cosine identity (original crop vs enhanced, `analysis.identity_similarity`, the same method as the calibration harness) and, below `IDENTITY_MIN` (0.5 — wants GPU calibration; validation's different-person floor is 0.363), retries once at halved creativity and keeps the best-scoring face. The similarity + attempts are recorded per face in the job metadata (`metadata.json` diagnostics, alongside the seam scan that was previously dropped). Backstop by design — the face pass at creativity 0.10 is already ~0.92, so it mostly measures. Still open: gating close-up faces that ride the main pass (measure-only, since retrying the whole pass is expensive), and calibrating the threshold on a golden set.
4. Real post-processing — ~~wavelet color match~~ DONE 2026-07-16. ~~Calibrated sharpening + halo suppression + seam inspection~~ DONE 2026-07-21: gradient-gated unsharp (flat regions keep their noise unamplified) with the anti-ringing clamp to the 3x3 erode/dilate envelope — overshoot past local extremes IS the halo, so none can form; Laplacian detail +40% on the validation outputs with no visible halo at the hardest edge. Seam scan compares coherent full-length discontinuity fractions in output vs reference (magnitude-based scoring false-flagged every crisp generative edge); zero false positives on validation outputs, catches a 6-gray-level synthetic seam; diagnostics only — logs + state.artifacts, never fails the job. Still open: grain matching for protected regions (deterministic patches read softer than enhanced surroundings).
